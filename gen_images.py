...
def get_hero_image_filename(name):
    ...
    return Path("../../Dropbox/Photos/Marvel_Champions_Tier_List/Heroes/modded") / filename
...
def hero_plays(statistics):
    ...
    tab = Table(d, column_definitions=[
        ...
        #ColumnDefinition(name="Image", title="", textprops={"ha":"right"}, width=1, plot_fn=image),
...
def villain_plays(statistics):
    #convert different dictionary for panda consumption
    frame_dict = { villain[1].name: [
                #                  get_hero_image_filename(hero[1].name),
                                  villain[1].total_plays, 
                                  villain[1].win_percentage, 
                                  villain[1].difficulty_data.standard1_win_percentage,
                                  villain[1].difficulty_data.expert1_win_percentage,
                                  villain[1].aspect_data.aspect_plays["Justice"].win_percentage,
                                  villain[1].aspect_data.aspect_plays["Leadership"].win_percentage,
                                  villain[1].aspect_data.aspect_plays["Aggression"].win_percentage,
                                  villain[1].aspect_data.aspect_plays["Protection"].win_percentage
                                  ] for villain in statistics.sorted_villains}
    d = pd.DataFrame.from_dict(frame_dict, orient='index', columns =[#"Image",
                                                                     "Plays", 
                                                                     "Win %", 
                                                                     "S1", 
    ...
    # see https://matplotlib.org/stable/users/explain/colors/colormaps.html for colormap options
    tab = Table(d, column_definitions=[
        ColumnDefinition(name="index", title="Villains", width=2, textprops={"ha":"left", "weight":"bold"}),
        #ColumnDefinition(name="Image", title="", textprops={"ha":"right"}, width=1, plot_fn=circled_image),
        ColumnDefinition(name="Plays", group="Overall"),
        ColumnDefinition(name="Win %", group="Overall", formatter=decimal_to_percent, cmap=matplotlib.cm.YlGn),
        ColumnDefinition(name="S1", group="Difficulty Win %", formatter=decimal_to_percent, cmap=matplotlib.cm.BuGn),
